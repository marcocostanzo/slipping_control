...
void sub_ls_combined(const slipping_control_msgs::LSCombinedStamped::ConstPtr& msg)
{
    ...
    // y_kf[0] = 0.0336;
    // input_vector[0] = y_kf[0];
    // ss_info.f_max_0 = 2.0*0.0283;
    // // ss_info.f_max_0 = 0.0335;

    // ss_info.beta_o2 = M_PI* pow(msg->radius,4)*betaA*(0.5 + pow(msg->cor_tilde,2));
    // ss_info.beta_o2 = 0.0200*(0.5 + pow(msg->cor_tilde,2));
    // ss_info.beta_o2 = 0.0400*(0.5 + pow(msg->cor_tilde,2));
    // ss_info.beta_o2 =  M_PI* pow(0.00246,4)*betaA*(0.5);

    ss_info.beta_o2 = 0.0300*(0.5 + pow(msg->cor_tilde,2));
...
int main(int argc, char *argv[]){
    ...
    ros::init(argc, argv, "one_dof_observer");
